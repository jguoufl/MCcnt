function [xp vp Ektp xpo vpo Ektpo]=scattering(XI,E1,flags,fd,dwire,xp,vp,Ektp,xpo,vpo,Ektpo)
    
%% output, (xp vp Ektp) the position, verlicty, and kinetic energy of particles
%%         (xpo vpo Ektpo) for the other band.
global t_step Egh1 vF
global Evec ga_ap ga_op ga_ii ga_tot tau_IR hw            % scattering rate
global kfd Nxf
    ...
    while noAll>0             
        ...
        if length(mRest1)>0       
            ...
            if noAll>0
                ...
                ga_ii_mScat=interp1(Evec,ga_ii,Ektp(mScat)); % impact ionization rate
                ga_tot_mScat=ga(mScat);                      % total scattering rate
                ga_self_mScat=ga_tot_mScat-ga_ap_mScat-ga_op_mScat-ga_ii_mScat; % the self scattering rate
                ...
                mii=mScat(find((rand_sca>((ga_ap_mScat+ga_op_mScat+ga_IR_mScat)./ga_tot_mScat)) &...
                    (rand_sca<=((ga_ap_mScat+ga_op_mScat+ga_IR_mScat+ga_ii_mScat)./ga_tot_mScat)))); % IR emission
                ...
                if (length (mOp)>0)   
                 ...
                 % 2.3.4 impact ionization in mii
                 if length(mii)>0
                     Eg=2*Egh1;
                     Esuplus=Ektp(mii)-Eg; % surplus E equaly divided between 3 particles                    
                     Ektp(mii)=max(0, Esuplus./3);            % carrier kinetic energy
                     vp(mii)=sign(vp(mii)).*vE(Ektp(mii),Egh1,vF);  % carrier velocity
                     %% add new carriers generated by impact ionization
                     % to the same band
                     ind=length(xp)+1:length(xp)+length(mii); 
                     xp(ind)=xp(mii);
                     Ektp(ind)=Ektp(mii);
                     vp(ind)=vp(mii);
                     % to the other band
                     ind=length(xpo)+1:length(xpo)+length(mii); 
                     xpo(ind)=xp(mii);
                     Ektpo(ind)=max(0, Esuplus./3);
                     vpo(ind)=vp(mii);
                     
                 end
                 %%% end of impact ionization in mii
            end         % if noAll>0
